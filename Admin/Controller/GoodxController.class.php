...
class GoodxController extends CommonController {
	

    //产品分类
    public function goodx_sort(){
        $goods=new \Model\GoodsModel;
        $list=array();
        $list[0] = $goods->where(array("category"=>"近视镜片","Maker"=>$_SESSION['ID']))->count();
        $list[1] = $goods->where(array("category"=>"太阳镜","Maker"=>$_SESSION['ID']))->count();
        $list[2] = $goods->where(array("category"=>"隐形眼镜","Maker"=>$_SESSION['ID']))->count();
        $list[3] = $goods->where(array("category"=>"老花镜","Maker"=>$_SESSION['ID']))->count();
        $list[4] = $goods->where(array("category"=>"隐形眼镜护理液","Maker"=>$_SESSION['ID']))->count();
        $list[5] = $goods->where(array("category"=>"眼镜架","Maker"=>$_SESSION['ID']))->count();
    ...
    public function goodx_add(){
        if(!isset($_POST['password'])){
            ...
            if($password!=$res['password']){
               $this->error('密码错误', U('Goodx/goodx_sort'), 1);
            }
            ...
            if($res){
                $this->success('添加成功', U('Goodx/goodx_sort'), 1);
            }else{
                $this->error('添加失败');
            }
        }
    ...
    public function goodx_details(){
        ...
        if (isset($_POST['password'])) {
            ...
            $res=$user->find($ID);
            if($password!=$res['password']){
               $this->error('密码错误', U('Goodx/goodx_list?category='.$_POST["category"]), 1);
            }
            ...
            if(isset($_POST['1'])){
                if($_POST['pi']=="0"){
                    ...
                    $r=$goods->save($data);
                    if($r!==false){
                        $this->success('修改成功', U('Goodx/goodx_list?category='.$_POST["category"]), 1);
                    
                    }else{
                        $this->error('修改失败', U('Goodx/goodx_list?category='.$_POST["category"]), 1);
                    } 
                }elseif ($_POST['pi']=="1") {
                    $upload = new \Think\Upload();// 实例化上传类
                    $upload->maxSize   =     3145728 ;// 设置附件上传大小
                ...
                if($res){
                    ...
                    if($ret){
                        ...
                        $this->success('删除成功', U('Goodx/goodx_list?category='.$_POST["category"]), 1);
                    }else{
                        $this->error('删除失败', U('Goodx/goodx_list?category='.$_POST["category"]), 1);
                    }
                }else{
                    $this->error('删除失败', U('Goodx/goodx_list?category='.$_POST["category"]), 1);
                }
